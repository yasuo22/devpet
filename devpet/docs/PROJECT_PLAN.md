...
### M11 - Codex 真实数据 + 猫粮交易 + 成长系统（第六步）✅
- **Codex token 接入**：`codex.js` 支持 API 自动拉取 / 手动上报真实 token 消耗，钱包持久化
- **猫粮购买交易系统**：`catfood.js` 重构为 token 货币 → 购买 4 种档次猫粮（基础/三文鱼/金枪鱼/和牛，不同单价）
- **宠物成长系统**：`growth.js` 亲密度 / 经验 / 等级 / 称号，喂食 / 互动 / 专注均提升成长
- **等级解锁高档猫粮**：三文鱼 Lv.3 / 金枪鱼 Lv.6 / 和牛 Lv.10
